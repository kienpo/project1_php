...
$query = "SELECT * FROM courses WHERE 1 "; // Truy vấn cơ bản
        ...
        <div class="container">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <!-- Nút thêm sinh viên -->
                    <?php if (checkPrivilege('\?module=course&action=add_course')) { ?>
                        <a href="?module=course&action=add_course" class="btn btn-success btn-sm fs-5">Thêm học phần<i class="fa-solid fa-plus ms-2"></i></a>
                    <?php } ?>
                </div>
                <!-- Thanh tìm kiếm -->
                <form method="GET" action="">
                    <input type="hidden" name="module" value="course">
                    <input type="hidden" name="action" value="view_course">
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <?php if (checkPrivilege('\?module=course&action=update_course&course_id=[0-9]*')) { ?>
                                    <td><a href="<?php echo _WEB_HOST; ?>?module=course&action=update_course&course_id=<?php echo $item['course_id']; ?>" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></a></td>
                                <?php } ?>
                                <?php if (checkPrivilege('\?module=course&action=delete_course&course_id=[0-9]*')) { ?>
                                    <td><a href="<?php echo _WEB_HOST; ?>?module=course&action=delete_course&course_id=<?php echo $item['course_id']; ?>" onclick="return confirm('Bạn có chắc muốn xóa?')" class="btn btn-warning btn-sm"><i class="fa-solid fa-trash"></i></a></td>
                                <?php } ?>
                            </tr>
                        <?php
                        endforeach;
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="alert alert-danger text-center">Không có học phần nào</div>
                            </td>
                        </tr>
                    <?php
                    endif;
            ...
            <div class="pagination">
                <?php
                $query = "SELECT COUNT(*) FROM courses WHERE 1 ";
                if (!empty($search)) {
                    $query .= "AND (course_name LIKE :search OR course_code LIKE :search)";
                }
                $stmt = $conn->prepare($query);
                ...
                if ($page >= 2) {
                    echo "<a href='?module=course&action=view_course&page=" . ($page - 1) . "'>Prev</a>";
                }

                for ($i = 1; $i <= $total_pages; $i++) {
                    if ($i == $page) {
                        $pagLink .= "<a class='active' href='?module=course&action=view_course&page=" . $i . "'>" . $i . "</a>";
                    } else {
                        $pagLink .= "<a href='?module=course&action=view_course&page=" . $i . "'>" . $i . "</a>";
                    }
                }
                echo $pagLink;

                if ($page < $total_pages) {
                    echo "<a href='?module=course&action=view_course&page=" . ($page + 1) . "'>Next</a>";
                }
                ?>
            </div>
        </div>
